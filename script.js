...
let state = {
  ...
  view: 'grid',
  filters: { cat: 'all', minRating: 0, maxDistance: null, search: '', tags: [] },
  sort: 'prominence',
  userLocation: null,
};
...
function getCatConfig(place) {
  return CAT_CONFIG[place.category || guessCategory(place)] || { label: 'Wisata', color: '#64748b', icon: 'bi-geo-alt' };
}
function guessCategory(place) {
  ...
  if (t.includes('taman')||t.includes('curug')||t.includes('danau')) return 'nature';
  return 'cultural';
}

function getPhotoUrl(place, maxW = 600) {
  if (place.photo_url) return place.photo_url;
  const cats = { nature:'2d9e6b,134e30', beach:'0ea5e9,0369a1', mountain:'7c5c3b,3d2e1e', cultural:'e88a22,b45309', historical:'9b4f96,5b2168' };
...
function getFavs() { try { return JSON.parse(localStorage.getItem('tl_favs')||'[]'); } catch { return []; } }
function isFav(id) { return getFavs().includes(id); }
function toggleFav(id, btn) {
  let favs = getFavs();
  if (favs.includes(id)) { favs = favs.filter(f => f !== id); btn.innerHTML = '<i class="bi bi-heart"></i>'; }
  else { favs.push(id); btn.innerHTML = '<i class="bi bi-heart-fill fav-active"></i>'; }
...
function createPlaceCard(place, mode = 'grid') {
  ...
  const dist = place.distance_from_purwokerto ? `<span class="card-dist"><i class="bi bi-signpost-2 me-1"></i>${place.distance_from_purwokerto} km</span>` : '';
  const gemBadge = place.hidden_gem ? '<span class="gem-badge"><i class="bi bi-gem me-1"></i>Hidden Gem</span>' : '';
...
function openDetail(placeId) {
  ...
  const addr = place.formatted_address || place.vicinity || 'Banyumas, Jawa Tengah';
  ...
  const nearby = state.places.filter(p => {
    if (p.place_id === placeId) return false;
    const d = haversine(lat, lng, p.geometry?.location?.lat||0, p.geometry?.location?.lng||0);
    return d < 20000;
  }).slice(0, 3);
  ...
  const nearbyBlock = nearby.length ? `
    <div class="detail-nearby mt-4">
      ...
      <div class="row g-3">${nearby.map(p => `
        <div class="col-12">
  ...
  const mapEmbed = CONFIG.GOOGLE_MAPS_API_KEY !== 'YOUR_GOOGLE_MAPS_API_KEY'
...
function initExplorePage() {
  ...
  if (q) { trackSearch(q); const el = document.getElementById('filterSearch'); if(el) el.value = q; }
  if (cat) setActiveCategory(cat);

  state.places = DB.getPlaces();
  ...
  // Rating range
  const rr = document.getElementById('ratingRange'), rv = document.getElementById('ratingVal');
  if (rr) rr.addEventListener('input', () => rv && (rv.textContent = rr.value == 0 ? 'Semua' : `${rr.value}★+`));
  ...
  // Live search with tracking
  const fs = document.getElementById('filterSearch');
  if (fs) fs.addEventListener('input', debounce(() => {
...
function setActiveCategory(cat) {
  document.querySelectorAll('.cat-filter-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.cat === cat));
}

function applyFilters() {
  const rr = document.getElementById('ratingRange'), ds = document.getElementById('distanceFilter');
  state.filters.minRating = rr ? parseFloat(rr.value) : 0;
  state.filters.maxDistance = ds ? (ds.value ? parseInt(ds.value) : null) : null;
  ...
  if (state.filters.cat !== 'all') places = places.filter(p => (p.category||guessCategory(p)) === state.filters.cat);
  if (state.filters.minRating > 0) places = places.filter(p => (p.rating||0) >= state.filters.minRating);
  // Hidden gem filter
  if (state.filters.gemOnly) places = places.filter(p => p.hidden_gem);
  if (state.filters.maxDistance && state.userLocation) {
    places = places.filter(p => {
      const loc = p.geometry?.location; if (!loc) return true;
      return haversine(state.userLocation.lat, state.userLocation.lng, loc.lat, loc.lng) <= state.filters.maxDistance;
    });
  }
...
function sortResults() {
  ...
  else if (state.sort === 'name') places.sort((a,b) => a.name.localeCompare(b.name));
  else if (state.sort === 'distance') places.sort((a,b) => (a.distance_from_purwokerto||99)-(b.distance_from_purwokerto||99));
  state.filtered = places;
  state.currentPage = 1;
  renderGrid();
}
...
function renderGrid() {
  const grid = document.getElementById('placeGrid'), info = document.getElementById('resultsInfo');
  const total = state.filtered.length, start = (state.currentPage-1)*state.perPage;
  ...
  grid.innerHTML = state.filtered.slice(start, start+state.perPage).map(p => createPlaceCard(p, state.view)).join('');
...
function renderPagination(total) {
  ...
  for (let i=1;i<=pages;i++) {
    if (i===1||i===pages||Math.abs(i-state.currentPage)<=1) html += `<li class="page-item ${i===state.currentPage?'active':''}"><button class="page-link" onclick="goPage(${i})">${i}</button></li>`;
    else if (Math.abs(i-state.currentPage)===2) html += `<li class="page-item disabled"><span class="page-link">…</span></li>`;
  }
...
function goPage(p) { state.currentPage = p; renderGrid(); window.scrollTo({top:200,behavior:'smooth'}); }
function setView(v) {
  state.view = v;
...
function resetFilters() {
  state.filters = { cat:'all', minRating:0, maxDistance:null, search:'', tags:[] };
  const rr=document.getElementById('ratingRange'), rv=document.getElementById('ratingVal');
  const fs=document.getElementById('filterSearch'), ds=document.getElementById('distanceFilter');
  if(rr) rr.value=0; if(rv) rv.textContent='Semua'; if(fs) fs.value=''; if(ds) ds.value='';
  document.querySelectorAll('.cat-filter-btn').forEach(b => b.classList.toggle('active', b.dataset.cat==='all'));
  state.filtered=[...state.places]; state.currentPage=1; renderGrid();
}
...
function loadFeatured() {
  ...
  state.places = DB.getPlaces();
  const places = [...state.places].sort((a,b) => (b.rating||0)-(a.rating||0)).slice(0,6);
  grid.innerHTML = places.map(p => createPlaceCard(p)).join('');
}

// ── User location ─────────────────────────────────────────────
function getUserLocation() {
  const status = document.querySelector('#locationStatus span'); if (!navigator.geolocation) return;
...
function getSearchHistory() {
  try { return JSON.parse(localStorage.getItem(SEARCH_STORE_KEY)||'[]'); } catch { return []; }
}

function getTopKeywords(n = 10) {
  const freq = {};
  getSearchHistory().forEach(({ q }) => q.split(/\s+/).forEach(t => { if (t.length>=2) freq[t]=(freq[t]||0)+1; }));
  return Object.entries(freq).sort((a,b)=>b[1]-a[1]).slice(0,n).map(([word,count])=>({word,count}));
}

function loadSmartTrending() {
  const top = getTopKeywords(4), wrap = document.getElementById('trendingChips'); if (!wrap||top.length<2) return;
  wrap.innerHTML = '<span class="chip-label"><i class="bi bi-fire me-1"></i>Untukmu:</span>' +
    top.map(({word}) => `<a href="explore.html?q=${encodeURIComponent(word)}" class="chip chip-personal">${word}</a>`).join('');
}

function loadPersonalRecommendations() {
  const top = getTopKeywords(3); if (!top.length) return;
  const all = DB.getPlaces(), kws = top.map(k=>k.word);
  const matched = all.filter(p => kws.some(kw => (p.name+' '+(p.description||'')+' '+p.category+' '+(p.tags||[]).join(' ')).toLowerCase().includes(kw))).slice(0,3);
  if (!matched.length) return;
  const sec=document.getElementById('recommendSection'), grid=document.getElementById('recommendGrid');
...
function loadHiddenGems() {
  const grid = document.getElementById('hiddenGemGrid'); if (!grid) return;
  const gems = DB.getPlaces().filter(p => p.hidden_gem).slice(0, 3);
  if (!gems.length) { document.getElementById('hiddenGemSection')?.classList.add('d-none'); return; }
...
function openItineraryModal() {
  const modal = document.getElementById('itineraryModal'); if (!modal) return;
...
function renderItineraryOptions() {
  const wrap = document.getElementById('itineraryOptions'); if (!wrap) return;
  wrap.innerHTML = ITINERARY_PRESETS.map(p => `
...
function buildItinerary(presetId) {
  const preset = ITINERARY_PRESETS.find(p => p.id === presetId); if (!preset) return;
  const allPlaces = DB.getPlaces();
  const places = preset.slots.map(id => allPlaces.find(p => p.place_id === id)).filter(Boolean);
  const kuliner = preset.kuliner.map(id => KULINER_DATA.find(k => k.id === id)).filter(Boolean);

  const result = document.getElementById('itineraryResult'), opts = document.getElementById('itineraryOptions');
  if (!result) return;
  opts?.classList.add('d-none');

  let totalHtm = 0;
  places.forEach(p => { if (p.htm?.weekday) totalHtm += p.htm.weekday; });
  kuliner.forEach(k => { if (k.htm) totalHtm += k.htm; });

  const schedule = [];
  // Morning: 2 places
  places.slice(0,2).forEach((p,i) => schedule.push({ time: TIMES[i], type: 'wisata', data: p }));
  // Midday kuliner
  if (kuliner[0]) schedule.push({ time: '12:30', type: 'kuliner', data: kuliner[0] });
  // Afternoon: 1 place
  if (places[2]) schedule.push({ time: TIMES[2], type: 'wisata', data: places[2] });
  // Late afternoon kuliner
  if (kuliner[1]) schedule.push({ time: '16:00', type: 'kuliner', data: kuliner[1] });
...
function loadAdminMessages() {
  ...
  if (!msgs.length) { list.innerHTML='<div class="p-4 text-center text-muted"><i class="bi bi-inbox display-6 d-block mb-2"></i>Belum ada pesan masuk.</div>'; return; }
...
function markRead(i) {
  const msgs = JSON.parse(localStorage.getItem('tl_messages')||'[]');
  if (msgs[i]) { msgs[i].read=true; localStorage.setItem('tl_messages',JSON.stringify(msgs)); loadAdminMessages(); }
}
function clearMessages() { if(!confirm('Hapus semua pesan?'))return; localStorage.removeItem('tl_messages'); loadAdminMessages(); }

function loadSearchAnalytics() {
  const body=document.getElementById('searchAnalyticsBody'), statsBody=document.getElementById('searchStatsBody'), recBody=document.getElementById('recommendPreview');
  if (!body) return;
  const top = getTopKeywords(15), history = getSearchHistory();
  if (!top.length) { body.innerHTML='<p class="text-muted text-center py-3">Belum ada data pencarian.</p>'; return; }
  const max = top[0].count;
  body.innerHTML = top.map(({word,count},i)=>`
  ...
  if (recBody) {
    const kws=top.slice(0,3).map(k=>k.word), all=DB.getPlaces();
    const prev=all.filter(p=>kws.some(kw=>(p.name+' '+(p.description||'')+' '+p.category).toLowerCase().includes(kw))).slice(0,3);
    recBody.innerHTML = prev.length
      ? '<p class="text-muted small mb-2">Destinasi yang akan direkomendasikan:</p>'+prev.map(p=>`<div class="d-flex align-items-center gap-2 mb-2"><img src="${getPhotoUrl(p,80)}" style="width:40px;height:40px;border-radius:8px;object-fit:cover" alt=""/><div><div class="fw-600 small">${p.name}</div><div class="text-muted" style="font-size:.75rem">${p.category}</div></div></div>`).join('')
      : '<p class="text-muted small">Belum ada rekomendasi yang cocok.</p>';
  }
}
function clearSearchHistory() { if(!confirm('Reset data pencarian?'))return; localStorage.removeItem(SEARCH_STORE_KEY); loadSearchAnalytics(); }
